...
Additionally, full resource replacement requires us to do extra work when implementing APIs. For example, we now need to check if the user tried to
modify any number of immutable fields.
...
As of now, we live in a twilight world. Some production APIs adhere to [RFC 2616](https://datatracker.ietf.org/doc/html/rfc2616) (full resource replacement) but most break with it to be more practical.
Here are two examples: [Paypal](https://developer.paypal.com/docs/api/invoicing/v2/#templates_update), [Twillio](https://www.twilio.com/docs/sendgrid/api-reference/users-api/update-your-username).
...
**PATCH to the rescue**: Some popular production APIs converted all their mutations from PUT to PATCH to allow for more flexibility. Here are two examples:
[Pinterest](https://developers.pinterest.com/docs/api/v5/pins-update), [Notion](https://www.postman.com/notionhq/notion-s-api-workspace/request/l3gvnpr/update-a-database). Others support PATCH and PUT at the same time.
