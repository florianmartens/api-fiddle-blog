...
In the wild, we often see PATCH requests to update a subset of resource fields. Let's take our example from above. If we don't which to 
replace the entire user object but only the first name we may send the following request:
...
[Http Specification RFC 2616](https://datatracker.ietf.org/doc/html/rfc2616). Instead, it is a proposed standard tracked in 
...
to the fields of our record (like `firstName` or `lastName`) but can author any set of instructions meant to change the state of our record.

**A simple example:** Let's imagine our API manages a set of counters. We send requests to increment a counter identified by an `id`.
...
As you can see, this PATCH request is not idempotent. We are introducing a side effect.
Sending the same request many times will generate a different response on every request.

In contrast, partially updating our `firstName` from our user record (s. above) was performed idempotently.

PATCH requests can be idempotent but they don't have to be.
